...
The UI can be changed with code from `server()`.

## Updating Inputs

Every input control `{type}Input()` is paired with an update function `updateTextInput()` that allows to modify the control after it has been created, e.g., to reset parameters back to their initial values, or to hierarchically create select boxes. There are two issues:

- Modifications of UI take some time, use `freezeReactiveValue()` to tell all downstream calculations that an input value is currently stale.
